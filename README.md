...
It generates an EC SECP256R1 **Robobo Root Certificate Authority (RoboboRootCA)**, device SSL/TLS certificates, individual and combined PKCS#12 (`.p12`) keystores, and JSON manifests for client devices (such as Android smartphones/tablets).
...
- **PKCS#12 Keystores**: Export individual `.p12` files per robot and combined `.p12` bundles containing fleet identity keys and Root CA certificates.
...
- **Android Integration Suite**: Includes Java helper classes (`android/`) for importing PKCS#12 keystores and manifests into Android `SSLContext` / `KeyManagerFactory`.
...
Idempotently processes RoboboRootCA, robot certificates, PKCS#12 keystores, and JSON manifest:
```bash
...
- **[SSLContextFactory.java](file:///D:/RemoteWork/robobo-pki/android/SSLContextFactory.java)**: Builds `SSLContext` from native `.p12` identities and `ca.crt` Root CA streams.
...
- **`ca.crt`**: The Robobo Root CA certificate (`output/RoboboRootCA.crt`).
- **`<robot_id>.p12`** (or `robobo-identities.p12`): Native PKCS#12 identity containing client certificate and private key (e.g., `output/pkcs12/rob-7vh.p12`).
- **`manifest.json`**: The JSON manifest used for looking up robot hostnames, URLs, and availability.
- **`keystore_password.txt`**: The password for opening the PKCS#12 keystore(s).


> **💡 Why avoid BKS?**
> BKS (Bouncy Castle KeyStore) generation and parsing can cause heavy overhead and thread hanging on Android. Using native PKCS#12 (`.p12`) and X.509 (`ca.crt`) assets works natively in Android out-of-the-box (`KeyStore.getInstance("PKCS12")`), loading in milliseconds without external dependencies or UI freezes.
...
- `SSLContextFactory.createSSLContextFromP12(...)` creates a `TrustManager` directly from the `ca.crt` stream in `res/raw`. The app automatically trusts all Robobo fleet robots securely out-of-the-box!
